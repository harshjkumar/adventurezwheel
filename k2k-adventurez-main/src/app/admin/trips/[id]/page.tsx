...
interface ItineraryDay { day: number; title: string; description: string; overnight?: string; distance?: string; altitude?: string; isRoute?: boolean }
...
export default function EditTripPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addItineraryDay = () =>
    setItinerary([...itinerary, { day: itinerary.length + 1, title: "", description: "", overnight: "", distance: "", altitude: "", isRoute: false }]);
            ...
            {itinerary.map((day, i) => (
              <div key={i} className="border border-gray-200 rounded-lg p-4 space-y-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-gray-50">
                  <input 
                    type="checkbox" 
                    checked={day.isRoute || false} 
                    onChange={(e) => updateItinerary(i, "isRoute", e.target.checked)} 
                    className="w-4 h-4 accent-accent" 
                  />
                  <span className="text-sm font-medium text-charcoal">Mark as route line (will show only route in header, bold, and collapsed by default)</span>
                </label>
              </div>
            ))}
          </>
