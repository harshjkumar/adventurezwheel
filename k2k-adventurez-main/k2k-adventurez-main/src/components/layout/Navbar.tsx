...
export function Navbar({ trips = [] }: { trips?: Trip[] }) {
  ...
  const navLinks = [
    ...
    { label: "Spiti Valley", href: "/spiti-valley", items: spitiTrips },
    { label: "Other Destinations", href: "/trips", items: otherTrips },
    { label: "About Us", href: "/about" },
    { label: "Blog", href: "/blogs" },
  ];

  // Pages that need solid navbar for readability
  const forceSolid =
    ['/trips', '/faq', '/policies/cancellation', '/policies/privacy'].includes(pathname) ||
    ...
    <>
      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
          <div className="flex items-center justify-between h-16 lg:h-[72px]">
            {/* Logo — left */}
            <Link href="/" prefetch={true} onClick={() => { setMobileOpen(false); setSearchOpen(false); }} className="flex-shrink-0 group flex items-center justify-center">
              <img
                src="/Untitled%20(1).png"
                alt="K2K Adventurez"
                className="h-20 lg:h-[96px] w-auto object-contain transition-transform duration-300 group-hover:scale-105"
              />
            </Link>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <div className="fixed inset-0 pointer-events-none flex items-center justify-center opacity-[0.06]">
              <img
                src="/Untitled%20(1).png"
                alt=""
                aria-hidden="true"
                className="w-[300px] sm:w-[380px] md:w-[460px] max-w-[88vw] h-auto object-contain"
              />
            </div>
            
            <div className="min-h-full flex flex-col justify-center py-24 md:py-32">
              <nav className="flex flex-col items-center gap-6 relative z-10 w-full px-4">
              {navLinks.map((link, i) => (
                <motion.div
                  ...
                  <div className="flex items-center justify-center gap-2 mb-1">
                    ...
                    {link.items && link.items.length > 0 && (
                      <button
                        ...
                        className={`p-2 rounded-full bg-stone-50 text-charcoal/50 transition-all duration-300 ${activeMobileCategory === link.label ? "rotate-180 bg-accent/10 text-accent" : ""}`}
                      >
                        <ChevronDown size={20} />
                      </button>
                    )}
                    ...
                    <AnimatePresence>
                      {activeMobileCategory === link.label && (
                        <motion.div
                          ...
                          className="overflow-hidden flex flex-col items-center gap-3 py-4 w-full bg-stone-50/50"
                        >
                          {link.items.map(trip => (
                            <Link
                              key={trip.id}
                              href={`/trips/${trip.slug}`}
                              onClick={() => setMobileOpen(false)}
                              className="font-nav text-sm text-charcoal/70 hover:text-accent transition-colors text-center px-4 py-1"
                            >
                              {trip.displayTitle || trip.title}
                            </Link>
                          ))}
